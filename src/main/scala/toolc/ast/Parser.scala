      ...
      currentToken.kind match {
        ...
        case IDKIND =>
          ...
          currentToken.kind match {
            ...
            case DOT =>
              val a = currentToken.kind match {
                case DOT =>
                  eat(DOT)
                  currentToken.kind match {
                    case LENGTH =>
                      eat(LENGTH)
                      parseNext(new ArrayLength(id))
                    case OR =>
                      eat(OR)
                      eat(LPAREN)
                      val args = parseArgumentsMethod
                      eat(RPAREN)
                      parseNext(new MethodCall(id, new Identifier("||"), args))
                    case AND =>
                      eat(AND)
                      eat(LPAREN)
                      val args = parseArgumentsMethod
                    ...
                    case _ =>
                      currentToken match {
                        case x: ID =>
                          ...
                          val args = parseArgumentsMethod
                          eat(RPAREN)
                          parseNext(new MethodCall(id, new Identifier(x.value), args.reverse))
                        case _ =>
                          expected(IDKIND)
                      }
                  }
                case LBRACKET =>
                  ...
                  val index = parseExpression
                  eat(RBRACKET)
                  parseNext(new ArrayRead(id, index))
                case _ => id
              }
              if (currentToken.kind == SEMICOLON) {
                eat(SEMICOLON)
            ...
            case _ => expected(EQSIGN, LBRACKET, DOT)
          }
        case _ =>
