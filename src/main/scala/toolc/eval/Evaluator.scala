...
class Evaluator(ctx: Context, prog: Program) {
    ...
    def evalExpr(ectx: EvaluationContext, e: ExprTree): Value = e match {
        ...
        case And(lhs, rhs) => BoolValue(lhs) && BoolValue(rhs)
        case Or(lhs, rhs)  => BoolValue(lhs) || BoolValue(rhs)
        case Plus(lhs, rhs) => IntValue(lhs) + IntValue(rhs)
        case Minus(lhs, rhs) => IntValue(lhs) - IntValue(rhs)
        case Times(lhs, rhs) => IntValue(lhs) * IntValue(rhs)
        case Div(lhs, rhs) => IntValue(lhs) / IntValue(rhs)
        case LessThan(lhs, rhs) => IntValue(lhs) < IntValue(rhs)
        case Not(expr) => !BoolValue(expr)
        case Equals(lhs, rhs) =>
